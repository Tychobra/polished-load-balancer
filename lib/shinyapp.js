      ...
      .then(port => new Promise((resolve, reject) => {
        ...
        this.child.stderr.on('data', data => {
          ...
          if (this.status === 'STARTING' && hold.includes('Listening on')) {
            this.status = 'RUNNING';
            setTimeout(() => resolve('Shiny app instance started'), 1000);
          }
...
export class ShinyApp {
  ...
  constructor(config) {
    ...
    // every minute (make this longer in future),  check nTotalSessions of each instance, and
  ...
  */
  async getSession (id=null) {
    ...
    if (id && this.sessions[id]) {
      ...
      if (this.sessions[id].deleteTimer) {
  ...
  closeSession (id) {
    if (this.sessions[id]) {
      ...
      this.sessions[id].deleteTimer = setTimeout(() => this.deleteSession(id), 900 * 1000) 
