  ...
  async start () {
    return portsPool.getNext()
      .then(port => new Promise((resolve, reject) => {
        ...
        this.child = spawn(
          this.config.RscriptPath || '/usr/lib/R/bin/Rscript', 
          ['-e', 'Sys.setenv(\'INSTANCE_ID\'=\'' + this.id + '\') && shiny::runApp(appDir=\'' + this.config.appDir + '\', port=' + port + ', host=\'0.0.0.0\')']
        )
  ...
  async getSession (id=null) {
    ...
    if (id && this.sessions[id]) {
      ...
      if (instanceMinSessions.nActiveSessions < (this.config.maxSessions) - 1) {
        ...
        // maxSessions, so preemtively start a new instance to handle any future sessions.
        ...
        return this.startInstance(newInstanceId).then(() => {
          ...
          this.sessions[id] = {
            sessionId: id,
            //startedAt: (new Date()).getTime(),
            proxyUrl: `http://0.0.0.0:${this.children[newInstanceId].port - 1}`,
            //closeTimer: setTimeout(() => this.closeSession(id), 1800*1000),
            status: "ACTIVE",
            //closeSession: () => this.closeSession(id),
